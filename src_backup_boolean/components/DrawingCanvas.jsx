...
const ShapeObject = ({ shapeProps, isSelected, onSelect, onChange, canvasScale }) => {
    ...
    const commonProps = {
        ...
        stroke: '#ffffff',
        strokeWidth: 4 / canvasScale, // Prevent border from getting infinitesimally small
...
const DrawingCanvas = ({ selectedFilm, shapes, setShapes, activeShapeId, setActiveShapeId }) => {
    const containerRef = useRef();
    const [stageSize, setStageSize] = useState({ width: 0, height: 0 });
    const [scale, setScale] = useState(1);

    // Calculate highest Y point (length used) BEFORE calculating virtualCanvasHeight
    let maxY = 0;
    ...
    const billableLength = Math.max(500, Math.ceil(Math.max(maxY, 0) / 500) * 500);
                ...
                >
                    <Layer>
                        <Group x={offsetX * scale} y={0} scaleX={scale} scaleY={scale}>
                            ...
                            {/* Active Area Overlay (Semi-transparent) */}
                            {billableLength > 0 && (
                                <Rect
                                    x={0}
                                    y={0}
