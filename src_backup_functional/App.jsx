...
import { exportShapesToDXF, downloadDXF, importDXFtoShapes } from './utils/dxfExport';
...
function App() {
  ...
  const handleMergeShapes = (type) => {
    ...
    // Do not allow merging/subtracting WITH dxf shapes either
  ...
  const calculateMaxLength = () => {
    ...
    shapes.forEach(shape => {
      ...
      else if (shape.type === 'path') bottom += (shape.height || 0) / 2 * currentScaleY; // Path is correctly centered via offsetX/Y in DrawingCanvas so it only dips height/2 below shape.y
      ...
      else if (shape.type === 'bubble') bottom += 100 * currentScaleY; // Bubble starts at top-left, spans 100 downward
  ...
  const handleExportDXF = () => {
    ...
    if (dxfString) {
      downloadDXF(dxfString, 'film_cut_pattern.dxf');
    } else {
      alert("DXF 변환 중 오류가 발생했습니다.");
    }
  };
  ...
  const handleImportDXF = (e) => {
    ...
    // Reset target value so we can re-import the same file directly
    e.target.value = null;
  };
